...
if uploaded_file:
    ...
    # Only three columns per your request
    ...
    for r in range(data_start, data_end + 1):
        ...
        # row complete across H..P: complete if every cell not blank and not "X"
        row_complete = True
        for c in req_cols:
            v = ws.cell(row=r, column=c).value
            if v in (None, "", "X"):
    ...
    # Write rows (Center/Campus, Completion Rate of Enrollment, Is 100%?)
    row_i = 2
    for center_name, stats in sorted(center_stats.items(), key=lambda x: x[0]):
        ...
        rate_cell = ws_summary.cell(row=row_i, column=2, value=rate)
    ...
    # Chart: Complete Rate of Enrollment (blue bars, campus names as labels)
    if last_row >= 2:
        chart = BarChart()
        chart.type = "col"
        chart.title = "Complete Rate of Enrollment"
